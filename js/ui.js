...
export function updateDataAnalysisCharts() {
    ...
    statCardsContainer.innerHTML = `
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">総半荘数</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${totalHanchans}</p><p class="text-sm text-gray-400">開催日数: ${gameDays}日</p></div>
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">現時点トップ</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${leader.name}</p><p class="text-sm text-gray-400">${leader.totalPoints.toFixed(1)} pt</p></div>
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">平均着順トップ</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${topAvgRankPlayer.name}</p><p class="text-sm text-gray-400">${topAvgRankPlayer.avgRank.toFixed(2)}位</p></div>
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">1半荘最高素点</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${highestHanchan.name}</p><p class="text-sm text-gray-400">${highestHanchan.score.toLocaleString()}点</p></div>
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">１日獲得Ptトップ</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${highestDaily.name}</p><p class="text-sm text-gray-400">+${highestDaily.pt.toFixed(1)} pt</p></div>
        <div class="cyber-card p-3 flex flex-col justify-between"><p class="text-sm text-blue-400 font-bold uppercase tracking-wider">参加半荘数トップ</p><p class="text-2xl lg:text-3xl font-bold my-1 text-green-400 glowing-text">${topHanchansPlayer.name}</p><p class="text-sm text-gray-400">${topHanchansPlayer.totalHanchans}半荘</p></div>
    `;
